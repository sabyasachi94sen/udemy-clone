              ...
              {storeAcademicData && storeAcademicData.map((val, index) => (
                <tr key={index} className="border-b-[1.5px] border-gray-50 border-b-[#EDEDED] bg-gray-50">
                  <td
                    ...
                    onClick={()=>setTable(val && val?.student && val.student?.id,val && val?.student && val.student?.student_name)}
                    >
                    {val && val?.student && val.student?.student_name}
                  </td>
                  <td className="h-[7vh]">{val && val?.student && val.student?.date_of_birth}</td>
                  <td className="h-[7vh]">{val && val?.student && val.student?.country_of_citizenship}</td>
                  <td className="h-[7vh] text-center">{val && val?.account_manager && val.account_manager.username}</td>
