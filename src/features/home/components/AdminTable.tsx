              ...
              <table className="border-solid w-[88%] mx-auto relative left-2 font-sans font-bold -mt-1  bg-gray-50 text-[#344054] break-all">
                ...
                <tbody className="overflow">
                  {adminData.map((val,index)=><tr className="border-b-[1.5px] border-b-[#EDEDED] border-gray-50">
                    ...
                    <td className="w-[13%] h-[7vh] pl-6">{val.email}</td>
                    <td className="w-[13%] h-[7vh]">{val.update}</td>
                    {val.status=="Active"?<td className="w-[13%] h-[7vh] text-[#20A464] pl-3">{val.status}</td>:<td className="w-[13%] h-[7vh] pl-3">{val.status}</td>}
                    <td className="w-[13%] h-[7vh] pl-14">{val.student}</td>
                    <td className="w-[13%] h-[7vh] pl-10">{val.performance}</td>
                    <td className="w-[13%] h-[7vh] pl-20 cursor-pointer"><img alt="edit-icon" src="/images/edit.png" onClick={()=>onClick2(index)} /></td>
                    <td className="w-[13%] h-[7vh] pl-10 cursor-pointer"><img alt="delete-icon" src="/images/delete.png" onClick={()=>onClick3(index)}/></td>
