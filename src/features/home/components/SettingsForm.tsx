import Link from "next/link"

import { Button } from "@/shared/components"


export function SettingsForm(){
    return (
      <div className="bg-white w-[40%] h-[90vh] pt-6 pl-24 pr-24 rounded-lg shadow-lg bg-cyan-100">
        
        <div className="mb-12">
          <img alt="user-image" className="w-[150px] h-[150px] rounded-[50%] border-white border-2 block mx-auto" src="data:image/jpeg;base64,"/>
        </div>

        <div className="mb-1">
          <p className="mb-4 font-sans font-medium text-black font-sans">Name</p> 
          <input className="w-full py-2 bg-cyan-200 text-gray-500 px-1 rounded-lg outline-none mb-4"  name="name" type="text"/>
        </div>
         
        <div className="mb-1">
          <p className="mb-4 font-sans font-medium text-black font-sans">Email</p> 
          <input className="w-full py-2 bg-cyan-200 text-gray-500 px-1 rounded-lg outline-none mb-4"  name="email" type="email"/>
        </div>

        <div className="mb-1">
          <p className="mb-4 font-sans font-medium text-black font-sans">Organisation</p> 
          <input className="w-full py-2 bg-cyan-200 text-gray-500 px-1 rounded-lg outline-none mb-4"  name="organisation" type="text"/>
        </div>

        <div className="mb-1">
          <p className="mb-4 font-sans font-medium text-black font-sans">Password</p> 
          <input className="w-full py-2 bg-cyan-200 text-gray-500 px-1 rounded-lg outline-none mb-4"  name="password" type="password"/>
        </div>

      
        <Link href="">
          <a className="text-black font-bold font-sans" href="#">Change your password info here.</a>
        </Link>
         


        <div className="mx-auto mb-28 mt-10 flex justify-center">
          <Link href="/">
            <Button className="bg-cyan-400 text-white font-bold py-2 rounded-md w-[90%] h-[6vh] font-bold text-[21px]">Save my changes</Button>
          </Link>
        </div>
      </div>
    )
}